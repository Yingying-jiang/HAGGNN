...
class AvgPooling(nn.Module):
    ...
    def forward(self, x, pos, seq, ori):
...
class ShellWeightMLP(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, shell_layer):
        shell_layer = shell_layer.unsqueeze(1).float()
        weights = self.mlp(shell_layer)
        mask_1 = (shell_layer == 1).float()
        mask_2 = (shell_layer == 2).float()
        mask_3 = (shell_layer >= 3).float()
        max_weight = torch.max(weights)
        min_weight = torch.min(weights)
        mid_weight = (max_weight + min_weight) / 2
        weights = mask_1 * max_weight + mask_2 * mid_weight + mask_3 * min_weight
        return weights
...
class CDConv(MessagePassing):
    def __init__(self, r: float, l: float, kernel_channels: list[int], in_channels: int, out_channels: int, add_self_loops: bool = True, **kwargs):
        ...
        W_size = kernel_channels[-1] * in_channels, out_channels
        self.W = nn.Parameter(torch.randn(648, 27))
        self.add_self_loops = add_self_loops
        self.shell_weight_mlp = ShellWeightMLP()
    ...
    def forward(self, x: OptTensor, pos: Tensor, seq: Tensor, ori: Tensor, batch: Tensor, water_shell_layers: Tensor) -> Tensor:
        ...
        out = self.propagate(edge_index, x=(x, None), pos=(pos, pos), seq=(seq, seq), ori=(ori.reshape((-1, 9)), ori.reshape((-1, 9))), 
                             water_shell_layers=(water_shell_layers, water_shell_layers), size=None)
    ...
    def message(self, x_j: Optional[Tensor], pos_i: Tensor, pos_j: Tensor, seq_i: Tensor, seq_j: Tensor, ori_i: Tensor,
                ori_j: Tensor, water_shell_layers_i: Tensor, water_shell_layers_j: Tensor) -> Tensor:
        pos = pos_j - pos_i
